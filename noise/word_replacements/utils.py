...
# load assorted mistakes data;
# a dict of vocab along with the number of possible replacement candidates
...
# load mistakes mappings
# a dict of vocab along with their possible replacement candidates
    ...
    for line in tqdm(original_sentences):
        ...
        for word in words:
            ...
            if word in mistakes_vocab:
                try: overlap_words[word]+=1
                except: overlap_words[word]=1
    overlap_count = sum([*overlap_words.values()])
    overlap_percent = 100*overlap_count/total_count
    print("\n")
    print(f"unique tokens overlapped: {len(overlap_words)}")
    print(f"total tokens overlapped: {overlap_count}")
...
def noisyfy_word_tokens(original_sentences, mistakes_vocab, mistakes_mappings, expected_prob: "float btw 0 and 1", print_stats = True):
    print("total lines in inp to noisyfy_word_tokens: {}".format(len(original_sentences)))
    print("total tokens in inp to noisyfy_word_tokens: {}".format(sum([len(line.strip().split()) for line in original_sentences])))
    print("------------------------------------")
    overlap_words, overlap_count, total_count, overlap_percent = _calculate_mistaketoken_overlap(original_sentences, mistakes_vocab, return_mode=True)
    ...
    print("{:.4f}% of overlapped tokens will get replaced to match the total % of misspellings to {:.4f}%".format(100*prob,100*expected_prob))
    get_noisy_token = lambda token: random.choice(mistakes_mappings[token]) \
                                    if (token in mistakes_vocab and \
                                        random.uniform(0,1)<=prob and \
                                        len(token)>1) \
                                    else token
    error_original_pair = [
    ...
    if print_stats:
        ...
        print("Percentage of tokens that actually got replaced {}/{}={:.4f}%".format(sum(difflen),sum(originallen),100*sum(difflen)/sum(originallen)))
        # how many unique tokens did we replace?
        diffvocabdict = {}
        for (error,original) in error_original_pair:
            for a,b in zip(error.split(),original.split()):
                if a!=b:
                    try: diffvocabdict[b]+=1
                    except: diffvocabdict[b]=1
