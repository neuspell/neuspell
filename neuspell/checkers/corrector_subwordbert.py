...
from .seq_modeling.subwordbert import (
    load_model,
    load_pretrained,
    model_predictions,
    model_inference
...
class BertChecker(Corrector):
    ...
    def correct_strings(self, mystrings: List[str], return_all=False) -> List[str]:
        self.is_model_ready()
        mystrings = \
            bert_tokenize_for_valid_examples(mystrings, mystrings,
                                             self.bert_pretrained_name_or_path)[0]
    ...
    def from_huggingface(self, bert_pretrained_name_or_path, vocab: Union[Dict, str]):
        ...
        self.model = load_model(self.vocab,
                                bert_pretrained_name_or_path=self.bert_pretrained_name_or_path)
    ...
    def finetune(self,
        ...
        if START_EPOCH > 1:
            progress_write_file = (
                open(os.path.join(CHECKPOINT_PATH, f"progress_retrain_from_epoch{START_EPOCH}.txt"),
                     'w')
            )
        ...
        for epoch_id in range(START_EPOCH, N_EPOCHS + 1):
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(train_data_iter):
                ...
                batch_labels_, batch_sentences_, batch_bert_inp, batch_bert_splits = \
                    bert_tokenize_for_valid_examples(batch_labels, batch_sentences,
                                                     self.bert_pretrained_name_or_path)
                ...
                if batch_id % 10000 == 0:
                    ...
                    model.eval()
                    with torch.no_grad():
                        _, batch_predictions = model(batch_bert_inp, batch_bert_splits,
            ...
            for batch_id, (batch_labels, batch_sentences) in enumerate(valid_data_iter):
                ...
                model.eval()
                with torch.no_grad():
                    batch_loss, batch_predictions = model(batch_bert_inp, batch_bert_splits,
                                                          targets=batch_labels)
                model.train()
            ...
            if valid_acc >= max_dev_acc:
                ...
                print("Model saved at {} in epoch {}".format(os.path.join(CHECKPOINT_PATH, name),
                                                             epoch_id))
