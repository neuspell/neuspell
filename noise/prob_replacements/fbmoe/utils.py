...
############################################
# Helper functions
# Obtain least common subsequence with a 
#   modified criterion as compared to edit-distance
############################################
...
############################################
# Helper functions
# Methods to inject noises to clean sentences
#   take input 
#       a list of clean sentences and 
#       n-gram probabilities computed from a dataset
#   give output
#       a list of corrupted sentences
############################################
...
def noisyfy_backoff_homophones(stats, lines, alphas, homophones={}, topk=-1, lower=False, print_data=False):
    ...
    print("total tokens in inp to noisyfy_backoff_homophones: {}".format(sum([len(line.strip().split()) for line in lines])))
    for lll, line in tqdm(enumerate(lines)):
        ...
        for ttt, token in enumerate(wtokens):
            ...
            top_k = topk
            if top_k==0:
                if len(token)<=3: top_k = np.random.choice([0,1],p=[0.9,0.1])
                elif 3<len(token)<=6: top_k = np.random.choice([0,1,2],p=[0.7,0.15,0.15])
                else: top_k = np.random.choice([0,1,2,3,4],p=[0.20,0.25,0.35,0.15,0.05])
...
def _get_replace_probs_all_contexts(stats, raw_context, is_beginning, 
        correct_char, alphas, print_stats=False, nascii=128):
    """
    ...
    for ln in range(0,len(raw_context)+1): #[0,1,2,3,...,len(context)]
        ...
        if alpha!=0:
            ...
            for replace_char,prob in probs_dict.items():
                if replace_char=="":  replace_char_probs[epsilon_index] += prob*alpha
                else: replace_char_probs[ord(replace_char)] += prob*alpha
    if print_stats: print(f"sum_alpha: {sum_alpha}")
    normalize_by = sum_alpha
    if sum(replace_char_probs)==0:
        if correct_char!="": replace_char_probs[ord(correct_char)] = 1
        else: replace_char_probs[epsilon_index] = 1
        normalize_by = 1
    else:
    ...
    if print_stats:
        ...
        for i,val in enumerate(replace_char_probs):
            if val!=0:
                if(i==nascii): replace_dict[""] = val
                else: replace_dict[chr(i)] = val
...
############################################
# Helper functions
# same as noisyfy_backoff_homophones() but with 
#   limited arguments
############################################


def noisyfy_backoff_homophones_SIMPLE(stats, lines, alphas, homophones, topk=-1):
    """
    # a simple version of noisyfy_backoff_homophones() method for experimentation
    """
    max_gram = len(alphas)-1
    homophones_set = set([*homophones.keys()])
    new_lines = []
    for lll, line in tqdm(enumerate(lines)):
        ...
        for ttt, token in enumerate(wtokens):
            ...
            for i, char in enumerate(token):
                ...
                mod_probs.append(replace_char_prob)
            if top_k>=0:
                replace_token, _ = __replace_only_topk(true_chars,mod_chars,mod_probs,top_k)
            else:
                replace_token = "".join(mod_chars)
            wtokens[ttt] = replace_token
        new_lines.append(" ".join(wtokens))
    return new_lines