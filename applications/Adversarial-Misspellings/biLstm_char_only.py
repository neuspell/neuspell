...
class BiLSTM():
    def build_model(self, nwords, nchars, ntags):
        ...
        self.char_lower_fwdLSTM = dy.VanillaLSTMBuilder(1, CHAR_EMB_SIZE, CHAR_EMB_SIZE, self.model)
        self.char_lower_bwdLSTM = dy.VanillaLSTMBuilder(1, CHAR_EMB_SIZE, CHAR_EMB_SIZE, self.model)


        self.char_upper_fwdLSTM = dy.VanillaLSTMBuilder(1, 2*CHAR_EMB_SIZE, CHAR_HID_SIZE, self.model)
        self.char_upper_bwdLSTM = dy.VanillaLSTMBuilder(1, 2*CHAR_EMB_SIZE, CHAR_HID_SIZE, self.model)
        ...
        self.W_sm = self.model.add_parameters((ntags, 2 * CHAR_HID_SIZE))  # Softmax weights
    ...
    def get_char_embeddings(self, word):
        ...
        char_fwd_init = self.char_lower_fwdLSTM.initial_state()
        char_fwd_embs = char_fwd_init.transduce(char_embs)
        char_bwd_init = self.char_lower_bwdLSTM.initial_state()
    ...
    def calc_scores(self, words, chars):
        dy.renew_cg()
        char_embs = [self.get_char_embeddings(chars[x]) for x in range(len(words))]
        fwd_init = self.char_upper_fwdLSTM.initial_state()
        fwd_embs = fwd_init.transduce(char_embs)
        bwd_init = self.char_upper_bwdLSTM.initial_state()
